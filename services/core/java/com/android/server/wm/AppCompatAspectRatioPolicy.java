...
class AppCompatAspectRatioPolicy {
    ...
    void resolveAspectRatioRestrictionIfNeeded(@NonNull Configuration newParentConfiguration) {
        ...
        final ConfigOverrideHint overrideHint =
                mActivityRecord.mAppCompatController.getSandboxingPolicy().getResolveConfigHint();
        ...
        if (!resolvedBounds.isEmpty() && !resolvedBounds.equals(parentBounds)) {
            ...
            mActivityRecord.mAppCompatController.getSandboxingPolicy().updateOverrideDisplayInfo();
    ...
    private boolean applyAspectRatio(Rect outBounds, Rect containingAppBounds,
        ...
        if (containingRatio - aspectRatioToApply > ASPECT_RATIO_ROUNDING_TOLERANCE) {
            if (mActivityRecord.mAtmService.shouldForceCutoutFullscreen(mActivityRecord.packageName)) {
